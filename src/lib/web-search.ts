...
/**
 * Performs a web search using the Gemini grounding tool (with a tool-less
 * fallback for models that don't support grounding). Returns the collected
 * content plus any source URLs, or null if the search failed.
 *
 * This is a reusable helper extracted from the web-search route so the chat
 * route can enrich answers in "expanded" mode without persisting a source.
 */
